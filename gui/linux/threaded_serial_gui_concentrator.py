...
class Window(Frame):
	...
	def init_window(self):
		...
		# valve 0 button
		self.valve0controlbtn = Button(self.valveframe0, text="OFF", command=lambda: self.valvetogglewrapper(0, 3))
		self.valve0controlbtn.grid(column=2, row=0)

		# valve 1 button
		self.valve1controlbtn = Button(self.valveframe1, text="OFF", command=lambda: self.valvetogglewrapper(1, 4))
		self.valve1controlbtn.grid(column=2, row=0)

		# valve 2 button
		self.valve2controlbtn = Button(self.valveframe2, text="OFF", command=lambda: self.valvetogglewrapper(2, 6))
	...
	def valvetogglewrapper(self, valve, power):
		try:
			if valve == 0:
				if irr.toggle_valve(valve, power):
